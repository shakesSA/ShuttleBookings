...
$out =$dbh->prepare("select * from client_table order by CR_date desc");
$out->execute;
...
<title>
PERSONALIZED TOURS AND TRANSFERS - Full Returns Report
</title>
...
<tr align="center">
<td width="100%"><font color="black"><b>REPORT ON ALL RETURNS</font></td>
</tr>
</table>
<br>
...
while (@row = $out->fetchrow_array)
		{
			...
			print "<tr bgcolor=aquamarine> <td>$row[2]</td> <td>$row[3]</td> <td>$row[8]</td> <td>$row[11]</td> <td>$row[12]</td> <td>$row[10]</td> <td>$row[19]</td>";
