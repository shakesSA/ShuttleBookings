...
$out =$dbh->prepare("select * from client_table where CD_date = '$C_date' order by CD_pickuptime");
$out->execute;
$rc = $dbh->disconnect;

$dbh = DBI->connect('DBI:mysql:PTT') || die "connection error: $!";
$out2 =$dbh->prepare("select * from client_table where CR_date = '$C_date' order by CR_pickuptime");
$out2->execute;
$rc = $dbh->disconnect;
