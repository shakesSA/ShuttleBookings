...
$out =$dbh->prepare("select * from client_table order by CD_date desc");
$out->execute;
...
<title>
PERSONALIZED TOURS AND TRANSFERS - Full Departures Report
</title>
...
<tr align="center">
<td width="100%"><font color="black"><b>REPORT ON ALL DEPARTURES</font></td>
</tr>
</table>
<br>
...
while (@row = $out->fetchrow_array)
		{
			...
			print "<tr bgcolor=aquamarine> <td>$row[2]</td> <td>$row[3]</td> <td>$row[1]</td> <td>$row[6]</td> <td>$row[7]</td> <td>$row[5]</td> <td>$row[19]</td>";
